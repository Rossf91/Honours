...
//    //
//    for (arcsim::util::MultiHistogramIter I(*HI);
//         !I.is_end();
//         ++I)
//    {
//      fprintf(stderr, "%s\n", (*I)->to_string().c_str());
...
namespace arcsim {
  namespace util {    
    ...
    //
    bool
    HistogramComparator::operator() (const Histogram* lhs,
    ...
    //
    uint32
    MultiHistogram::refill_historgram_allocation_pool()
    {
    ...
    //
    Histogram*
    MultiHistogram::new_histogram(uint32 idx)
    {
      arcsim::concurrent::ScopedLock lock(multihist_entries_mutex_);
      
      // Re-fill allocation pool if it is empty
      //
      if (multihist_entries_alloc_pool_.empty()) { refill_historgram_allocation_pool(); }
      ...
      // Set Histogram name
      //
      entry->set_name("multihistogram.entry");
    ...
    //      
    Histogram * const
    MultiHistogram::get_hist_ptr_at_index(uint32 idx)
    {
      Histogram* h = 0; 
      std::map<uint32,Histogram*>::const_iterator I;
    ...
    //
    bool
    MultiHistogram::index_exists(uint32 idx) const
    {
    ...
    //
    bool
    MultiHistogram::index_exists_in_histogram(uint32 idx_hist, uint32 idx_entry) const
    {
      std::map<uint32,Histogram*>::const_iterator I;
    ...
    //
    void
    MultiHistogram::inc(uint32 idx_hist, uint32 idx_entry)
    {
      Histogram* h = 0; 
      std::map<uint32,Histogram*>::const_iterator I;
      if ((I = multihist_map_index_value_.find(idx_hist)) != multihist_map_index_value_.end()) {
    ...
    void
    MultiHistogram::inc(uint32 idx_hist, uint32 idx_entry, uint32 val)
    {
      Histogram* h = 0; 
      std::map<uint32,Histogram*>::const_iterator I;
      if ((I = multihist_map_index_value_.find(idx_hist)) != multihist_map_index_value_.end()) {
    ...
    //
    void
    MultiHistogram::clear()
    {
      for (std::set<Histogram*>::const_iterator
           I = multihist_entries_.begin(),
           E = multihist_entries_.end();
           I != E; ++I)
      {
        (*I)->clear();
      }
    ...
    std::string
    MultiHistogram::to_string()
    {
      std::set<Histogram*,HistogramComparator> multihist_entries_sorted(multihist_entries_.begin(),
                                                                        multihist_entries_.end());

      std::ostringstream buf;
      
      buf << "[MULTIHIST:" << name_ << "]";
      
      for (std::set<Histogram*>::iterator
           I = multihist_entries_sorted.begin(),
           E = multihist_entries_sorted.end();
           I != E; ++I)
      {
    ...
    // Constructor
    //
    MultiHistogram::MultiHistogram(const char * name,
                                   uint32       alloc_size)
    : alloc_size_(alloc_size)
    {
      ...
      // init name
      //
      for (i = 0; i < kMultiHistogramMaxNameSize - 1 && name[i]; ++i)
    ...
    // MultiHistogramIter
    //
    MultiHistogramIter::MultiHistogramIter(const MultiHistogram& h)
    : hist(h)
    { 
    ...
    bool
    MultiHistogramIter::is_end()
    {
      return iter == entries.end();
    }
    
    bool
    MultiHistogramIter::is_begin()
    {
