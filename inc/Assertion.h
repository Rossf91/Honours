...
//
#define FATAL(err)                                                              \
  arcsim::Assertion(__FILE__, __LINE__).Fail("%s", err)

#define FATAL1(fmt, p1)                                                         \
  arcsim::Assertion(__FILE__, __LINE__).Fail(fmt, (p1))


#define UNIMPLEMENTED()                                                         \
  FATAL("UNIMPLEMENTED CODE")

#define UNIMPLEMENTED1(p1)                                                      \
  FATAL1("UNIMPLEMENTED CODE", p1)


#define UNREACHABLE()                                                           \
  FATAL("UNREACHABLE CODE")

#define UNREACHABLE1(p1)                                                        \
  FATAL1("UNREACHABLE CODE", p1)
...
//
#define ASSERT(cond) do {} while (0 && (cond))

#else

#define ASSERT(cond)                                                            \
  do {                                                                          \
    if (!(cond)) arcsim::Assertion(__FILE__, __LINE__).Fail("EXPECTED: %s", #cond);\
  } while (0)
